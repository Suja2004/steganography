...
import { encodeImage, BASE_URL } from "../api/api";
...
export default function EncodeForm() {
  ...
  const handleEncode = async (e) => {
    ...
    try {
      const data = await encodeImage(file, message);

      setEncodedUrl(`${BASE_URL}${data.encoded_image}`);
      setPlots({
        original: `${BASE_URL}${data.original_lsb}`,
        encoded: `${BASE_URL}${data.encoded_lsb}`,
        changes: `${BASE_URL}${data.changes_lsb}`,
      });
